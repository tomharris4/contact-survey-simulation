...
cm_age =  cm_age_sampled - cm_age_biased
diff_max = max([h for j in range(len(cm_age)) for h in cm_age[j]])
sb.heatmap(cm_age, ax=ax[0], cmap="RdBu", center = 0, vmin = -1 * diff_max,  vmax = diff_max, cbar_kws={'label': 'Mean # unique contacts \n per day'})

ax[0].invert_yaxis()

ax[0].set(title='$\Delta$[True - Biased]', xlabel="Participant Age", ylabel="Contact Age")
...
ax[1].set(title='Elderly (65+ years)')
ax[1].grid()
...
ax[2].scatter([r_1[5]],[ar_diff[5]],color='g',label='$b_{age}=1$')
# for i in range(10):
...
ax[2].set(title='Elderly (65+ years)')
ax[2].grid()
...
plt.tight_layout()
plt.savefig('../Figures/supp_bias_strength_age_' + pathogen + '_' + metric + '.pdf')
