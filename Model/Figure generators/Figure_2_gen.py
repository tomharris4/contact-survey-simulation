...
if not processed:
    ...
    sb.heatmap(cm_age, ax=ax[0,0], vmin = 0, cbar_kws={'label': 'Mean # unique contacts \n per day'})
else:
    cm_age = np.load('../Data/Contact matrices/' + input_network + '__full_pop__processed__'  + attr + '__' + context + '.npy')
    sb.heatmap(cm_age, ax=ax[0,0], vmin = 0, cbar_kws={'label': 'Per-capita contact rate'})
...
if not processed:
    ...
    sb.heatmap(cm_eth, ax=ax[1,0], vmin = 0, cbar_kws={'label': 'Mean # unique contacts \n per day'})
else:
    cm_eth = np.load('../Data/Contact matrices/' + input_network + '__full_pop__processed__'  + attr + '__' + context + '.npy')
    sb.heatmap(cm_eth, ax=ax[1,0], vmin = 0, cbar_kws={'label': 'Per-capita contact rate'})
...
if not processed:
    ...
    sb.heatmap(cm_race, ax=ax[0,1], vmin = 0, cbar_kws={'label': 'Mean # unique contacts \n per day'})
else:
    cm_race = np.load('../Data/Contact matrices/' + input_network + '__full_pop__processed__'  + attr + '__' + context + '.npy')
    sb.heatmap(cm_race, norm=LogNorm(), ax=ax[0,1],  cbar_kws={'label': 'Per-capita contact rate'})
...
if not processed:
    ...
    sb.heatmap(cm_ses, ax=ax[1,1], vmin = 0, cbar_kws={'label': 'Mean # unique contacts \n per day'})
else:
    cm_ses = np.load('../Data/Contact matrices/' + input_network + '__full_pop__processed__'  + attr + '__' + context + '.npy')
    sb.heatmap(cm_ses, ax=ax[1,1], vmin = 0, cbar_kws={'label': 'Per-capita contact rate'})
...
ax[1,1].xaxis.set_major_formatter(FixedFormatter(ses_dist_labels))
ax[1,1].yaxis.set_major_locator(MultipleLocator(1,offset=0.5))
ax[1,1].yaxis.set_major_formatter(FixedFormatter(ses_dist_labels))
ax[1,1].tick_params(which='major', pad=2, labelsize=7,labelrotation=45)
ax[1,1].set_yticklabels(labels=ses_dist_labels,va='center')
