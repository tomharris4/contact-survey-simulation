...
sb.heatmap(np.transpose(cm_race), ax=ax[0][0], cmap="RdBu", center = 0, vmin = -1 * diff_max,  vmax = diff_max, cbar_kws={'label': '$\Delta$Mean # unique contacts\nper day'})
    ...
    for k in range(10):
        for input_params in range(len(input_params_all)):
            for target_group in target_group_all:
                ...
                # ar_diff_overall[k] = ar_diff_overall[k] + ar_diff
